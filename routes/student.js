...
router.post(
  ...
  async (req, res) => {
    ...
    const student = new Student({ name, email, password, rollno, college });
...
// Route for getting the logged in student details
router.get("/", FetchStudent, async (req, res) => {
  const sid = req.student_id;

  const student = await Student.findOne(
    { _id: sid },
    { name: 1, email: 1, rollno: 1, college: 1, classrooms: 1, teachers: 1 }
  );
...
router.get("/classrooms/:status", FetchStudent, async (req, res) => {
  ...
  const c = student.classrooms.map((value) => {
    if (value.status === status) return value.id;
  });
...
router.get("/teachers", FetchStudent, async (req, res) => {
  ...
  const teacher = await teachers.find({ _id: { $in: student.teachers } }, {name:1, email:1});
...
router.put("/enrollme/:cid", FetchStudent, async (req, res) => {
  ...
  const result = await Student.exists({
    _id: req.student_id,
    "classrooms.id": id,
  });

  if (result) return res.send({ enrollStatus: 1 });

  // if enrollment is unique then update the classrooms of the student .
  const student = await Student.findOneAndUpdate(
    { _id: req.student_id },
    { $push: { classrooms: { id: id, status: "pending" } } }
  );
  res.send({ student });
});
